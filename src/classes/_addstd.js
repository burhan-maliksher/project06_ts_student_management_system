...
import { inputValidation, ageValidation, emailValidation, phoneValidation, } from "../_InputValitator.js";
...
export class _Student extends _Course {
    ...
    async _addStd_M() {
        ...
        let repeatRandomGenerator = false;
        let reg_no;
        do {
    ...
    async _enroll_M() {
        ...
        let std_id = parseInt(stdCredentials.id);
        ...
        if (this._stdLsit.find((item) => item.std_id == std_id)) {
            ...
            if (this._enrolledStdList.find(item => item.std_id == std_id)) {
                ...
                let enrollingStdDetails = this._stdLsit.find((item) => item.std_id == std_id);
                // getting details from course list against the user choice
                let courseDetail = this._course_list.find((item) => item.name == courseName);
